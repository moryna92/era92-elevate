...
const SC = { todo:'#4f9cf9', inprogress:'#d4820a', review:'#b97cf9', done:'#3ecf8e' };
const SL = { todo:'To Do', inprogress:'In Progress', review:'In Review', done:'Done' };
...
function defaultTasks() {
  ...
  return [
    ...
    { id:uid(), title:'Design session discussion guide',      desc:'Create a reusable template for book discussions',                assignee:'Peter Mwanja',       status:'review',     priority:'medium', tag:'Design',        due:'2026-03-28', createdBy:'Admin', createdAt:now },
    ...
    { id:uid(), title:'Create social media announcement',     desc:'Draft announcement post for the club launch',                   assignee:'Meshack',            status:'review',     priority:'low',    tag:'Marketing',     due:'2026-03-25', createdBy:'Admin', createdAt:now },
      ...
      {curView !== 'admin' ? (
        <div style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ display:'flex', gap:7, padding:'8px 18px', borderBottom:'1px solid var(--border)', flexShrink:0, overflowX:'auto' }}>
            {[[tasks.length,'Total','var(--text)'],[tasks.filter(t=>t.status==='todo').length,'To Do','var(--blue)'],[tasks.filter(t=>t.status==='inprogress').length,'In Progress','var(--amber)'],[tasks.filter(t=>t.status==='review').length,'In Review','var(--purple)'],[done,'Done','var(--green)']].map(([n,l,c])=>(
              ...
              return (
                <div key={col.id} style={{ width:264, flexShrink:0, display:'flex', flexDirection:'column' }}>
                  ...
                  <div onDragOver={e => e.preventDefault()} onDrop={() => dropToCol(col.id)}
                    ...
                    {ct.map(t => <TaskCard key={t.id} task={t} currentUser={currentUser} isAdmin={isAdmin} onEdit={openEdit} onDelete={delTask} onReassign={openReassign} onDragStart={id => dragRef.current=id} />)}
        ...
        <AdminPanel tasks={tasks} onAdd={openAdd} onEdit={openEdit} onDelete={delTask} onReassign={openReassign} onClear={clearAll} onRefresh={async()=>{const r=await storageGet(TASK_KEY);if(r?.tasks){setTasks(r.tasks);showToast('Refreshed ✓');}}} />
      )}
        ...
        <Modal onClose={() => setShowModal(false)}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            <FG label="Assigned To">
              ...
              <datalist id="tSuggest">{members.map(m=><option key={m} value={m}/>)}</datalist>
            </FG>
      ...
      {showReassign && (
        <Modal onClose={() => setShowReassign(false)}>
          ...
          <FG label="Reassign To">
            ...
            <datalist id="rSuggest">{members.map(m=><option key={m} value={m}/>)}</datalist>
          </FG>
...
// ── TASK CARD ────────────────────────────────────────────────
function TaskCard({ task: t, currentUser, isAdmin, onEdit, onDelete, onReassign, onDragStart }) {
  ...
  return (
    <div draggable onDragStart={() => onDragStart(t.id)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background:'var(--surface2)', border:`1px solid ${hov?'var(--border)':'var(--border2)'}`, borderRadius:7, padding:10, cursor:'grab', position:'relative', animation:'cardIn .2s ease', boxShadow: hov?'0 4px 18px rgba(0,0,0,.5)':'none' }}>
      <div style={{ width:3, position:'absolute', left:0, top:7, bottom:7, borderRadius:'0 2px 2px 0', background:pc }} />
      {canEdit && hov && (
      ...
      <div style={{ fontWeight:600, fontSize:12, lineHeight:1.4, marginBottom:4, paddingLeft:8, paddingRight: canEdit?50:8 }}>{t.title}</div>
      {t.desc && <div style={{ fontSize:11, color:'var(--text2)', lineHeight:1.5, marginBottom:6, paddingLeft:8 }}>{t.desc}</div>}
      {t.tag && <div style={{ paddingLeft:8, marginBottom:5 }}><span style={{ fontSize:9, padding:'2px 6px', borderRadius:8, fontFamily:'DM Mono,monospace', background:`${pc}20`, color:pc }}>{t.tag}</span></div>}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingLeft:8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:4, fontSize:10, color:'var(--text2)' }}>
          {t.assignee ? <>
            <div style={{ width:17, height:17, borderRadius:'50%', background:memberColor(t.assignee), color:'var(--ink)', fontSize:8, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Syne,sans-serif', flexShrink:0 }}>{getInitials(t.assignee)}</div>
            <span>{t.assignee.split(' ')[0]}</span>
          </> : <span style={{ color:'var(--text3)' }}>Unassigned</span>}
        </div>
        {t.due && <div style={{ fontSize:10, fontFamily:'DM Mono,monospace', color: overdue?'var(--red)':'var(--text3)' }}>{overdue?'⚠ ':''}{fmtDate(t.due)}</div>}
      </div>
    </div>
  );
}
...
// ── ADMIN PANEL ──────────────────────────────────────────────
function AdminPanel({ tasks, onAdd, onEdit, onDelete, onReassign, onClear, onRefresh }) {
  const members = [...new Set(tasks.map(t => t.assignee).filter(Boolean))];
  const done = tasks.filter(t => t.status === 'done').length;
  const pct  = tasks.length ? Math.round(done/tasks.length*100) : 0;
            ...
            <div key={m} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:8, overflow:'hidden', marginBottom:10 }}>
              ...
              <div style={{ borderTop:'1px solid var(--border)', padding:'10px 14px' }}>
                {mt.map(t => {
                  ...
                  const pc = PRIORITY_COLORS[t.priority] || '#555';
                  return (
                    <div key={t.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 8px', background:'var(--surface2)', borderRadius:5, fontSize:11, marginBottom:4 }}>
                      <div style={{ width:7, height:7, borderRadius:'50%', background:pc, flexShrink:0 }} />
                      <div style={{ flex:1, fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.title}</div>
