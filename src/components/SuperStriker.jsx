...
import { TEAM, INITIALS_MAP, PILLARS, SS_KEYS, PALETTE, memberColor, getInitials } from '../constants.js';
import { storageGet, storageSet } from '../storage.js';
import {
  getWeekId, getWeekNum, getPrevWeekId, getVotingStatus,
  shouldAnnounce, fmtCountdown, getStatusCountdown, getStatusLabel,
} from '../weekUtils.js';

const C = {
  ink:'#1a1208', paper:'#f5f0e8', amber:'#d4820a', amberL:'#f0b340',
  rust:'#b84c1e', sage:'#4a7c59', cream:'#faf6ef', mid:'#7a6a50', border:'#ddd4bc',
};

function blankScores() {
  return Object.fromEntries(TEAM.map(n => [n, { score:0, wins:0, reason:'No nominations yet' }]));
}
...
export default function SuperStriker({ currentUser, isAdmin, showToast }) {
  ...
  const sorted = TEAM.map(n => ({ name:n, ...(scores[n]||{score:0,wins:0,reason:'No nominations yet'}) })).sort((a,b)=>b.score-a.score);
  ...
  return (
    <div style={{ animation:'fadeUp .35s ease' }}>
      {/* HERO */}
      <div style={{ background:C.ink, padding:'24px 32px 18px', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:-60, right:-40, width:250, height:250, border:'38px solid rgba(212,130,10,.07)', borderRadius:'50%', pointerEvents:'none' }} />
        <div style={{ fontFamily:'DM Mono,monospace', fontSize:9, letterSpacing:3, textTransform:'uppercase', color:C.amber, marginBottom:7 }}>ERA92 Elevate · 2026 Performance Tracker</div>
        ...
        <p style={{ fontSize:12, color:'#a89880', marginTop:4 }}>6-pillar peer nomination · Auto-announced every Monday 7:30 AM</p>
        <div style={{ display:'flex', gap:7, marginTop:9, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <span style={{ background:'rgba(255,255,255,.06)', color:'#a89880', padding:'3px 11px', borderRadius:3, fontFamily:'DM Mono,monospace', fontSize:10 }}>{cdLabel}: {fmtCountdown(cd)}</span>
          <span style={{ fontFamily:'DM Mono,monospace', fontSize:10, color:'#5a4e3e' }}>WEEK {weekNum} · {new Date().getFullYear()} · {weekVotes.length} vote{weekVotes.length!==1?'s':''}</span>
        </div>
      </div>

      {/* NAV */}
      <div style={{ display:'flex', background:C.ink, borderBottom:'1px solid rgba(255,255,255,.06)', padding:'0 32px', overflowX:'auto' }}>
        {[['board','🏆 Leaderboard'],['vote','⚡ Nominate'],['history','📅 Winners'],['howto','ℹ️ How It Works'],
          ...
          <button key={id} onClick={() => setTab(id)} style={{
            ...
            color: tab===id?C.amberL:'#6a5c48', fontFamily:'DM Mono,monospace', fontSize:10,
      ...
      <div style={{ maxWidth:1000, margin:'0 auto', padding:'24px 18px 60px' }}>
        ...
        {tab === 'board' && (
          <div style={{ animation:'fadeUp .35s ease' }}>
            {/* stats strip */}
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', background:C.amber, borderRadius:4, overflow:'hidden', marginBottom:18 }}>
        ...
        {/* NOMINATE */}
        {tab === 'vote' && (
          <VoteTab weekNum={weekNum} weekId={weekId} votedMap={votedMap} submitVote={submitVote} status={status} cd={cd} cdLabel={cdLabel} C={C} />
        )}
        ...
        {/* SS ADMIN */}
        {tab === 'ssadmin' && isAdmin && (
          <SSAdmin weekNum={weekNum} weekVotes={weekVotes} scores={scores} C={C} forceAnnounce={forceAnnounce} resetWeek={resetWeek} />
        )}
      </div>
    </div>
  );
...
// ── VOTE TAB ─────────────────────────────────────────────────
function VoteTab({ weekNum, weekId, votedMap, submitVote, status, cd, cdLabel, C }) {
...
// ── SS ADMIN ─────────────────────────────────────────────────
function SSAdmin({ weekNum, weekVotes, scores, C, forceAnnounce, resetWeek }) {
  const tally = {};
  ...
  return (
    <div style={{ animation:'fadeUp .35s ease' }}>
      <div style={{ background:C.rust, color:'#fff', borderRadius:4, padding:'12px 18px', marginBottom:18, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:8, padding:16, marginBottom:16, display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:12, color:'var(--text2)' }}>Opens Mon 7:00 AM · Closes Fri 6:00 PM · Announces Mon 7:30 AM</div>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        <div style={{ background:C.cream, border:`1px solid ${C.border}`, borderRadius:4, padding:18 }}>
          ...
          <div style={{ background:'rgba(212,130,10,.08)', border:`1px solid ${C.amber}`, borderRadius:4, padding:12, marginTop:12 }}>
            ...
            <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
              <button onClick={forceAnnounce} style={{ background:C.sage, color:'#fff', border:'none', borderRadius:3, padding:'8px 13px', fontSize:11, fontFamily:'DM Mono,monospace', letterSpacing:1, textTransform:'uppercase', cursor:'pointer' }}>⚡ Announce Now</button>
            ...
            {weekVotes.length === 0
              ...
              : weekVotes.map((v, i) => (
                <div key={i} style={{ padding:'8px 0', borderBottom:`1px solid ${C.border}`, fontSize:12 }}>
                  ...
                  <div style={{ color:C.ink }}>→ <span style={{ background:'rgba(212,130,10,.1)', border:`1px solid ${C.amber}`, borderRadius:10, padding:'1px 9px', fontSize:10, fontFamily:'DM Mono,monospace' }}>{v.nominee}</span></div>
